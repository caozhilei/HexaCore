...
export default function NewAgentPage() {
  ...
  // Load shared memory spaces
  useEffect(() => {
    async function loadMemorySpaces() {
      try {
        const res = await fetch('/api/admin/memories', { credentials: 'include' })
        const payload = await res.json().catch(() => null)
        if (!res.ok) throw new Error(payload?.message || `Failed to load memory spaces (${res.status})`)
        setMemorySpaces((payload?.data || []).map((s: any) => ({ ...s, granted: false })))
      } catch (e: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Save memory grants if any selected
      const selectedSpaces = memorySpaces.filter((s) => s.granted)
      if (selectedSpaces.length > 0 && newAgent) {
        await fetch('/api/admin/memories/grants', {
          ...
          body: JSON.stringify({
            ...
            grants: selectedSpaces.map((s) => ({ space_id: s.id, permission: 'read' })),
          }),
        })
      }
