 ...
 * 测试辅助函数
 */
function createTestMessage(channel = 'whatsapp', accountId = 'business') {
    return {
        channel,
        accountId,
        peer: {
...
describe('ChannelManager', () => {
    ...
    beforeEach(() => {
        ...
        channelManager = new manager_1.ChannelManager(mockRedis, {
    ...
    describe('Session Key机制', () => {
        test('应该正确生成Session Key', async () => {
            ...
            expect(session.sessionKey).toBe(`agent:${agentId}:${message.channel}:${message.accountId}:direct:${message.peer.id}`);
        });
    ...
    describe('持久化', () => {
        test('应该持久化会话状态到Redis', async () => {
            ...
            const newManager = new manager_1.ChannelManager(mockRedis, {
                enablePersistence: true
            });
        ...
        test('禁用持久化时不应该保存到Redis', async () => {
            const disabledManager = new manager_1.ChannelManager(mockRedis, {
            ...
            // 创建启用了持久化的新管理器
            const enabledManager = new manager_1.ChannelManager(mockRedis, {
                enablePersistence: true
            });
            // 应该无法恢复会话
    ...
    describe('并发限制', () => {
        test('应该遵守最大并发会话限制', async () => {
            // 配置一个很小的限制便于测试
            const limitedManager = new manager_1.ChannelManager(mockRedis, {
                maxConcurrentSessions: 2
            });
            try {
                // 创建第一个会话
                const session1 = await limitedManager.createOrGetSession(createTestMessage(), 'general-agent');
                // 创建第二个会话
                const message2 = {
                    ...createTestMessage(),
                    peer: { id: '+8613898765432', kind: 'dm' }
                };
                const session2 = await limitedManager.createOrGetSession(message2, 'general-agent');
                // 尝试创建第三个会话（应该抛出错误）
                const message3 = {
                    ...createTestMessage(),
                    peer: { id: '+8613811122222', kind: 'dm' }
                };
                await expect(limitedManager.createOrGetSession(message3, 'general-agent')).rejects.toThrow('Maximum concurrent sessions limit reached');
            }
            finally {
    ...
    describe('统计信息', () => {
        test('应该正确跟踪会话统计', async () => {
            ...
            // 创建会话
            const session1 = await channelManager.createOrGetSession(createTestMessage(), 'general-agent');
        ...
        test('应该跟踪峰值并发数', async () => {
            ...
            const session2 = await channelManager.createOrGetSession(message2, 'general-agent');
