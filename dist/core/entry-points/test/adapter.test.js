...
const wecom_adapter_1 = require("../wecom-adapter");
const web_adapter_1 = require("../web-adapter");
/**
 * WhatsApp适配器测试
 */
...
describe('WeComAdapter', () => {
    let adapter;
    beforeEach(() => {
        adapter = new wecom_adapter_1.WeComAdapter();
    });
    test('should initialize with default config', () => {
...
describe('WebAdapter', () => {
    let adapter;
    beforeEach(() => {
        adapter = new web_adapter_1.WebAdapter();
    });
    test('should initialize with default config', () => {
...
describe('Adapter Performance', () => {
    test('should start within reasonable time', async () => {
        const adapter = new whatsapp_adapter_1.WhatsAppAdapter();
        const config = {
            enabled: true,
            adapter: 'baileys',
            config: {
    ...
    test('should send message with low latency', async () => {
        const adapter = new web_adapter_1.WebAdapter();
        const config = {
            enabled: true,
            adapter: 'widget',
            config: {
...
describe('Error Handling', () => {
    test('should handle connection errors gracefully', async () => {
        const adapter = new wecom_adapter_1.WeComAdapter();
        // 使用无效配置触发错误
        const invalidConfig = {
            enabled: true,
            config: {
