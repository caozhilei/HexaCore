...
class AuditLogger {
     ...
     * 记录容器创建事件
     */
    async logContainerCreated(containerId, agentId, sessionKey, sandboxMode, config) {
        const event = {
     ...
     * 记录容器销毁事件
     */
    async logContainerDestroyed(containerId, agentId, sessionKey, reason) {
        const event = {
            id: this.generateEventId(),
            timestamp: new Date().toISOString(),
     ...
     * 记录工具权限检查事件
     */
    async logToolPermissionCheck(toolName, context, result, metadata) {
        const event = {
            id: this.generateEventId(),
            timestamp: new Date().toISOString(),
     ...
     * 记录权限提升请求事件
     */
    async logElevationRequested(requestId, requester, toolName, riskLevel, reason) {
        const event = {
            id: this.generateEventId(),
            timestamp: new Date().toISOString(),
     ...
     * 记录权限提升批准事件
     */
    async logElevationApproved(requestId, approverId, expiresAt, reason) {
        const event = {
            id: this.generateEventId(),
            timestamp: new Date().toISOString(),
     ...
     * 记录权限提升拒绝事件
     */
    async logElevationDenied(requestId, approverId, reason) {
        const event = {
            id: this.generateEventId(),
            timestamp: new Date().toISOString(),
     ...
     * 记录策略违反事件
     */
    async logPolicyViolation(violationType, context, details) {
        const event = {
            id: this.generateEventId(),
            timestamp: new Date().toISOString(),
     ...
     * 记录异常检测事件
     */
    async logAnomalyDetected(anomalyType, context, severity, details) {
        const event = {
            id: this.generateEventId(),
            timestamp: new Date().toISOString(),
     ...
     * 记录安全警报事件
     */
    async logSecurityAlert(alertType, message, context, details) {
        const event = {
            id: this.generateEventId(),
            timestamp: new Date().toISOString(),
     ...
     * 查询审计事件
     */
    async queryEvents(filters, limit) {
     ...
     * 获取事件统计信息
     */
    async getEventStats(timeRangeHours = 24) {
        ...
        const recentAlerts = recentEvents.filter(event => event.eventType === 'security_alert' ||
            event.eventType === 'policy_violation').length;
        return {
    ...
    async checkAndTriggerAlert(event) {
        if (!this.config.alerting.enabled)
            return;
        const isCritical = this.config.alerting.criticalEvents.includes(event.eventType);
        if (isCritical) {
            await this.logSecurityAlert('critical_event', `Critical audit event detected: ${event.eventType}`, event.securityContext, { auditEvent: event });
    ...
    getEventSeverity(eventType) {
        ...
        if (highSeverity.some(type => eventType.includes(type)))
            return 'high';
        if (mediumSeverity.some(type => eventType.includes(type)))
            return 'medium';
        return 'low';
    }
    initializeStorage() {
