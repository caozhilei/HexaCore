...
exports.ToolPermissionFilter = void 0;
class ToolPermissionFilter {
     ...
     * 执行7层过滤链检查
     */
    async filterToolAccess(toolName, context, config) {
        const filters = [
     ...
     * 检查工具是否在允许的工具组内
     */
    async filterByToolProfile(toolName, context) {
     ...
     * 检查Provider级别的工具权限
     */
    async filterByProviderProfile(toolName, context) {
        // 模拟Provider级策略检查
     ...
     * 基于HexaCore.json全局配置
     */
    async filterByGlobalPolicy(toolName, context) {
        const policy = this.toolPolicies.get(toolName);
        if (!policy) {
     ...
     * 检查Provider认证状态
     */
    async filterByProviderPolicy(toolName, context) {
        // 模拟Provider认证检查
     ...
     * 应用Agent级别的个性化策略
     */
    async filterByAgentPolicy(toolName, context) {
        const policy = this.toolPolicies.get(toolName);
        if (!policy) {
        ...
        if (requiredCapabilities.length > 0) {
            const missingCapabilities = requiredCapabilities.filter(cap => !context.capabilities.includes(cap));
     ...
     */
    async filterByAgentProviderPolicy(toolName, context) {
        // 模拟Agent-Provider绑定检查
        // 在实际实现中，会检查Agent是否被授权使用特定的Provider
        const agentProviderBindings = {
     ...
     * 检查容器级别的工具权限
     */
    async filterBySandboxPolicy(toolName, context) {
        const policy = this.toolPolicies.get(toolName);
        if (!policy) {
     ...
     * 通过过滤器（占位符）
     */
    async passFilter(toolName, context) {
     ...
     * 获取工具所需的能力
     */
    getRequiredCapabilities(toolName) {
        const capabilityMap = {
...
exports.ToolPermissionFilter = ToolPermissionFilter;
