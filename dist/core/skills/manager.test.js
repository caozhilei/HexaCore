 ...
 * 创建测试技能目录
 */
async function createTestSkillDirectory(skillName, description) {
...
describe('技能管理器测试', () => {
    ...
    beforeEach(() => {
        skillManager = new manager_1.SkillManager(TEST_CONFIG);
    });
    afterEach(async () => {
        // 清理管理器
    ...
    test('管理器初始化', async () => {
        ...
        expect(skillManager.getStatus()).toBe(framework_1.SkillStatus.READY);
    ...
    test('创建和加载技能', async () => {
        ...
        // 重新初始化以发现新技能
        const newManager = new manager_1.SkillManager(TEST_CONFIG);
    ...
    test('执行技能', async () => {
        ...
        // 执行技能
        const result = await newManager.executeSkill('test-skill-2', { name: 'Tester' }, executeOptions);
        // 验证结果
...
`;
        ...
        const result = await newManager.executeSkill('restricted-skill', { command: 'test' }, executeOptions);
        expect(result.success).toBe(true);
    });
    /**
     * 测试5：技能卸载
    ...
    test('技能白名单和黑名单', async () => {
        ...
        const filteredManager = new manager_1.SkillManager(filteredConfig);
    ...
    test('错误处理 - 技能不存在', async () => {
        ...
        const result = await skillManager.executeSkill('non-existent-skill', { param: 'value' }, executeOptions);
    ...
    test('参数验证', async () => {
        ...
        // 测试缺少必填参数
        const result1 = await newManager.executeSkill('validation-skill', {}, // 缺少name参数
        executeOptions);
        expect(result1.success).toBe(false);
        expect(result1.error).toContain('验证失败');
        // 测试提供有效参数
        const result2 = await newManager.executeSkill('validation-skill', { name: 'Valid User' }, executeOptions);
    ...
    test('会话管理', async () => {
        ...
        const result = await newManager.executeSkill('session-skill', { name: 'Session User' }, executeOptions);
        expect(result.success).toBe(true);
    ...
    test('性能监控', async () => {
        ...
        const result = await newManager.executeSkill('performance-skill', { name: 'Performance Test' }, executeOptions);
