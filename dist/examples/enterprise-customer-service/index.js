...
exports.demonstrateHexadWorkflow = demonstrateHexadWorkflow;
...
async function demonstrateHexadWorkflow() {
    ...
    const wecomAdapter = new wecom_adapter_1.WeComAdapter();
    ...
    // 第二步：路由层 - 智能分发
    console.log('\n2. 路由层：7级匹配规则路由到客服频道');
    const routingEngine = new engine_1.RoutingEngine();
    // 配置路由规则
    ...
    const channelManager = new manager_1.ChannelManager();
    ...
    // 第四步：技能层 - 技能加载与执行
    console.log('\n4. 技能层：加载客服技能并在沙箱中执行');
    const skillManager = new manager_2.SkillManager();
    // 注册客服技能
    const customerServiceSkill = new customer_service_1.CustomerServiceSkill();
    await skillManager.registerSkill('customer_service_skill', customerServiceSkill);
    // 第五步：沙箱层 - 安全隔离执行
    console.log('\n5. 沙箱层：创建安全执行环境');
    const sandboxManager = new manager_3.SandboxManager();
    ...
    const memoryCompactor = new storage_1.MemoryCompactor();
    // 存储原始消息
