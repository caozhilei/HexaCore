...
export interface PruningConfig {
  ...
  ttl: number; // 生存时间（秒）
  valueThreshold: number; // 价值阈值
  cacheOptimization: {
    enabled: boolean;
    targetHitRate: number;
    evictionPolicy: 'lru' | 'lfu' | 'ttl';
  };
  costReductionTarget: number; // 成本降低目标比例
}
...
export interface ValueMetrics {
  timeDecayScore: number; // 时间衰减得分
  frequencyScore: number; // 使用频率得分
  relevanceScore: number; // 相关性得分
  importanceScore: number; // 重要性得分
...
export class SessionPruner {
  ...
  async pruneSession(
    sessionKey: string,
    sessionData: SessionSegment[],
    configOverride?: Partial<PruningConfig>
  ...
  private identifyLowValueContent(
    ...
    const now = Date.now();
    const lowValueSegments: SessionSegment[] = [];
  ...
  private calculateValueMetrics(
    ...
    const frequencyScore = Math.min(
      Math.log(segment.accessCount + 1) / Math.log(100), // 假设最大100次访问
      1.0
    );
   ...
   * 计算相关性得分
   */
  private calculateRelevanceScore(segment: SessionSegment): number {
    // 基于元数据中的相关性标记
   ...
   * 计算重要性得分
   */
  private calculateImportanceScore(segment: SessionSegment): number {
    // 基于工具类型权重
    if (segment.toolType) {
   ...
   */
  private softPrune(
    originalSegments: SessionSegment[],
    lowValueSegments: SessionSegment[]
  ): PruningResult {
    ...
    const tokenReduction = lowValueSegments.reduce(
  ...
  private hardReplace(
    ...
    // 直接过滤掉低价值段
    const replacedSegments = originalSegments.filter(
      segment => !lowValueIds.has(segment.id)
    );
    ...
    const totalTokens = originalSegments.reduce(
      (sum, seg) => sum + seg.tokenCount,
      0
    );
  ...
  private hybridPrune(
    ...
    const segmentsForSummary: SessionSegment[] = [];
    const segmentsForRemoval: SessionSegment[] = [];
    ...
    const finalSegments = retainedSegments.filter(
      segment => !removalIds.has(segment.id)
    );

    const originalTokens = originalSegments.reduce(
      (sum, seg) => sum + seg.tokenCount,
      0
    );
    
    const finalTokens = finalSegments.reduce(
      (sum, seg) => sum + seg.tokenCount,
      0
    );
   ...
   * 生成段摘要
   */
  private generateSegmentSummary(segment: SessionSegment): any {
   ...
   * 判断段是否可被摘要化
   */
  private canBeSummarized(segment: SessionSegment): boolean {
    // 根据内容类型判断
    const content = segment.content;
  ...
  async analyzeCachePerformance(
    accessPatterns: Array<{ segmentId: string; timestamp: Date }>,
    ...
    const lastAccessTimes = new Map<string, Date>();
    ...
    let recommendedPolicy: 'lru' | 'lfu' | 'ttl';
    ...
    // 估计命中率提升
    const expectedHitRate = Math.min(
      currentHitRate * 1.2, // 预计提升20%
   ...
   * 计算访问频率方差
   */
  private calculateAccessVariance(accessCounts: number[]): number {
   ...
   * 计算成本节约报告
   */
  calculateCostSavingsReport(
    originalTokens: number,
    prunedTokens: number,
    tokenPricePerMillion: number = 20 // 假设$20/百万tokens
  ): {
    tokenSavings: number;
    costSavings: number;
    savingsPercent: number;
    roi: number; // 投资回报率（假设修剪操作有成本）
  } {
