...
import { MemoryStorage, ConversationTurn } from './storage';
import { ConversationCompactor } from './compaction';
import { SessionPruner } from './pruning';
import { MemorySidecar } from './sidecar';

// 模拟测试数据
const TEST_SESSION_KEY = 'test_session_001';
const TEST_TURNS: ConversationTurn[] = [
  {
    id: 'turn_001',
  ...
  test('应识别低价值内容', () => {
    const testSegments = [
      ...
      },
      {
        ...
        accessCount: 1,
        tokenCount: 30,
        category: 'tool_result' as const,
        toolType: 'execution',
      },
    ];

    const lowValueSegments = pruner['identifyLowValueContent'](
      testSegments,
      0.5, // 高阈值
      3600
    );
...
describe('MemorySidecar', () => {
  ...
  test('应处理对话历史并提取知识', async () => {
    const result = await sidecar.processConversationHistory(
      TEST_SESSION_KEY,
      TEST_TURNS
    );
  ...
  test('应进行语义搜索', async () => {
    ...
    const results = await sidecar.searchMemories(
      TEST_SESSION_KEY,
      '天气'
    );
...
describe('三层优化机制集成测试', () => {
  ...
  beforeAll(async () => {
    storage = new MemoryStorage({ dbPath: ':memory:' });
    await storage.initialize();
    ...
    sidecar = new MemorySidecar({ enabled: true });
    await sidecar.initialize();
  });
  ...
  test('应实现完整的三层优化流程', async () => {
    ...
    if (needsCompaction) {
      const history = await storage.getConversationHistory(TEST_SESSION_KEY);
      const compactionResult = await compactor.compactConversation(
        TEST_SESSION_KEY,
        history
      );
    ...
    const pruningResult = await pruner.pruneSession(
      TEST_SESSION_KEY,
      testSegments
    );
    ...
    // 4. 边车知识提取
    const sidecarResult = await sidecar.processConversationHistory(
      TEST_SESSION_KEY,
      TEST_TURNS
    );
