...
import { EmbeddingService } from './retrieval';
...
export interface StorageConfig {
  ...
  compactionThreshold: number; // 压缩触发阈值（上下文使用率）
  pruningTTL: number; // 修剪TTL（秒）
...
export class MemoryStorage {
   ...
   * 保存对话轮次
   */
  async saveConversationTurn(turn: ConversationTurn): Promise<void> {
    if (!this.db) throw new Error('存储引擎未初始化');
    
    await this.db.run(
      `INSERT INTO conversation_turns (id, session_key, role, content, timestamp, token_count, metadata)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [
        turn.id,
        turn.sessionKey,
  ...
  async getConversationHistory(
    ...
    const rows = await this.db.all(
      `SELECT id, session_key as sessionKey, role, content, timestamp, token_count as tokenCount, metadata
   ...
   * 计算会话的当前Token使用量
   */
  async calculateTokenUsage(sessionKey: string): Promise<number> {
    if (!this.db) throw new Error('存储引擎未初始化');
   ...
   * 检查是否需要压缩
   */
  async needsCompaction(sessionKey: string): Promise<boolean> {
   ...
   * 保存记忆条目
   */
  async saveMemoryEntry(entry: Omit<MemoryEntry, 'id' | 'lastAccessed' | 'accessCount'>): Promise<string> {
    if (!this.db) throw new Error('存储引擎未初始化');
   ...
   * 更新记忆条目的访问信息
   */
  async updateAccessInfo(memoryId: string): Promise<void> {
    if (!this.db) throw new Error('存储引擎未初始化');
  ...
  async searchMemories(
    ...
    // 获取该会话的所有记忆条目
    const rows = await this.db.all(
      `SELECT id, session_key as sessionKey, content, embedding, source_turn_id as sourceTurnId,
   ...
   * 清理过期记忆
   */
  async cleanupExpiredMemories(maxAgeDays: number = 30): Promise<number> {
    if (!this.db) throw new Error('存储引擎未初始化');
