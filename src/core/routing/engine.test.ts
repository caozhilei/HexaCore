...
import { RoutingEngine, RoutingEngineOptions } from './engine';
import { 
  simplifiedExampleRules, 
  createTestMessage,
  vipCustomerRule,
  pricingInquiryRule,
  techSupportRule,
...
import { InboundMessage, RoutingEngineConfig, HealthChecker } from './rules';

// Mock健康检查器
class MockHealthChecker implements HealthChecker {
  private healthyAgents = new Set<string>();
  
  setAgentHealth(agentId: string, healthy: boolean): void {
    if (healthy) {
  ...
  async getAgentHealthStatus(agentId: string): Promise<{
    healthy: boolean;
    score: number;
    lastCheck: Date;
  }> {
...
describe('路由引擎测试', () => {
  ...
  const testConfig: RoutingEngineConfig = {
    rules: simplifiedExampleRules
  };
  
  const engineOptions: RoutingEngineOptions = {
    enableCaching: true,
    cacheTTL: 3600,
  ...
  beforeEach(() => {
    ...
    engine = new RoutingEngine(testConfig, engineOptions);
  ...
  test('VIP客户规则匹配', async () => {
    ...
    expect(result.rulePriority).toBe(vipCustomerRule.priority);
  ...
  test('价格咨询规则匹配', async () => {
    ...
    expect(result.rulePriority).toBe(pricingInquiryRule.priority);
  });
  ...
  test('技术支持规则匹配', async () => {
    ...
    expect(result.rulePriority).toBe(techSupportRule.priority);
  });
  ...
  test('默认规则匹配', async () => {
    ...
    expect(result.rulePriority).toBe(defaultRule.priority);
  ...
  test('批量路由', async () => {
    ...
    // 启用并行匹配进行测试
    const parallelEngine = new RoutingEngine(testConfig, {
      ...engineOptions,
  ...
  test('配置重加载', async () => {
    ...
    // 创建新配置，禁用VIP规则
    const newConfig: RoutingEngineConfig = {
      rules: [
        { ...vipCustomerRule, enabled: false },
        pricingInquiryRule,
        techSupportRule,
        defaultRule
      ]
  ...
  test('统计信息收集', async () => {
    ...
    await engine.route(createTestMessage({
      channel: 'telegram',
      content: { text: '另一个测试' }
    }));
  ...
  test('确定性哈希路由', async () => {
    // 创建启用确定性路由的引擎
    const deterministicEngine = new RoutingEngine(testConfig, {
      ...engineOptions,
  ...
  test('自适应超时', async () => {
    // 测试自适应超时功能
    const adaptiveEngine = new RoutingEngine(testConfig, {
      ...engineOptions,
...
// 运行测试的辅助函数（用于演示）
export async function runRoutingTests(): Promise<void> {
  ...
  try {
    ...
    const pricingMessage = createTestMessage({
      channel: 'whatsapp',
      peer: {
        metadata: {
          tier: 'basic',
