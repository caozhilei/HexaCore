...
import { CompositeMatcher } from './matchers';
import { RoutingRuleRepository } from '../database/routing-repo';

/**
 * 路由引擎状态
 */
export enum RoutingEngineState {
  IDLE = 'idle',
  MATCHING = 'matching',
...
export interface RoutingEngineOptions {
  ...
  useDatabaseRules?: boolean; // 新增选项：是否从数据库加载规则
}

/**
 * 路由匹配缓存项
 */
interface RoutingCacheItem {
...
export class RoutingEngine {
  ...
  private matcher: CompositeMatcher;
  private healthChecker?: HealthChecker;
  private options: RoutingEngineOptions;
  private state: RoutingEngineState = RoutingEngineState.IDLE;
  private routingRepo?: RoutingRuleRepository;
  
  // 缓存相关
  private cache: Map<string, RoutingCacheItem> = new Map();
  private cacheHits = 0;
  ...
  constructor(config: RoutingEngineConfig, options?: RoutingEngineOptions) {
    ...
    if (this.options.useDatabaseRules) {
      this.routingRepo = new RoutingRuleRepository();
    ...
    try {
      ...
      if (dbRules && dbRules.length > 0) {
        ...
        const rules: RoutingRule[] = dbRules.map(r => ({
          ...
          priority: r.priority,
          match: r.match_condition as any, // Cast JSON to RoutingMatchConditions
   ...
   * 路由消息
   */
  async route(message: InboundMessage): Promise<RoutingResult> {
   ...
   * 批量路由消息
   */
  async routeBatch(messages: InboundMessage[]): Promise<RoutingResult[]> {
   ...
   * 重新加载配置
   */
  async reloadConfig(newConfig: RoutingEngineConfig): Promise<void> {
    this.state = RoutingEngineState.MATCHING;
  ...
  private initializeRuleIndex(): void {
    // 这里可以构建规则索引以加速匹配
    // 简化实现：暂时不需要复杂索引
  }
  
  private sortRulesByPriority(): RoutingRule[] {
  ...
  private async matchRules(
    ...
    matchedConditions: Partial<RoutingRule['match']>;
    score: number;
  }> {
    // 按优先级顺序尝试匹配
  ...
  private calculateOverallScore(scores: Record<string, number>): number {
    ...
    if (scoreKeys.length === 0) return 1.0; // 默认规则
  ...
  private createDefaultResult(message: InboundMessage): RoutingResult {
    // 查找默认规则
    const defaultRule = this.config.rules.find(
      rule => rule.enabled !== false && rule.match.default === true
    );
  ...
  private createFallbackResult(message: InboundMessage, originalRule: RoutingRule): RoutingResult {
    return {
  ...
  private applyDeterministicRouting(message: InboundMessage, agentId: string): string {
    // 简化的一致性哈希实现
    ...
    if (availableAgents.length === 0) return agentId;
  ...
  private updateStatistics(matchTime: number, success: boolean): void {
    this.totalMatches++;
